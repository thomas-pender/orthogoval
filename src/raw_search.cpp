...
std::unordered_set<AG, AG::hash_func> orbit{};
...
# endif
     if ( auto it = orbit.find(ag); it == orbit.end() ) {
          std::cout << ag << '\n' << std::flush;
          nvertices++;
          orbit.insert(std::move(ag));
     }
}
...
# if HAVE_PTHREAD
     ...
     for ( std::uint32_t i{0}; i < NTHREADS - 1; i++ ) {
          ...
          threads[i] = std::thread(thread_func, first, first + block_len, std::ref(canonical));
     }

     first = (block_len * (NTHREADS - 1)) + start;
     thread_func(first, end, std::ref(canonical));

     for ( auto & t : threads ) t.join();
# else
     thread_func(1, LEN, canonical);
# endif
     std::cout << nvertices << '\n';
